...
[
  ...
  global-life-distribution                ;; average of clustering coefficients of all turtles
...
;;      set theta_1 random-float 1.0
 ...
 ;     set weighting-history random-float 1
  ...
  ask turtles [
    ...
    establish-color]
  init-age-USA2007
  if timescale = "months" [ask turtles[set-initial-life-distribution-USA2007-months]]
  if timescale = "years" [ask turtles[set-initial-life-distribution-USA2007]]
    if timescale = "months-erick" [ask turtles[set-initial-life-distribution-USA2007-months-erick]]
...
to rewire-pref
  ...
  set list-dists map [? / (((position ? list-dists + 2) ^ degree-of-pref) * num-eff)] list-dists 
...
to set-life-distribution-USA2007 ;;Life expectation for ages according data colected by the Centers for Disease Control
   ...
   if selective-pressure = "death-probability" and cumulative-score <= social-threshold [
    ...
    let maximum-payoff [cumulative-score] of max-one-of turtles [cumulative-score]
    
    set life-distribution map [1 - ((cumulative-score / maximum-payoff) ^ power-of-selection) * (1 - ?) ] global-life-distribution] 
...
to set-life-distribution-USA2007-months-erick
...
to set-initial-life-distribution-USA2007-months-erick
  
  set global-life-distribution (list 0.0067375 0.000464 0.0002865 0.0002165 0.000174 0.0001575 0.000147 0.000137 0.000124 0.000107 9.45e-05 9.8e-05 0.0001325 0.0002045 0.000305 0.000415 0.000521 0.000621 0.000709 0.000785 0.000863 0.0009375 0.000988 0.001006 0.001 0.000987 0.000978 0.000976 0.0009865 0.0010085 0.001035 0.0010655 0.001104 0.00115 0.001207 0.0012735 0.001354 0.0014505 0.001566 0.0017 0.00185 0.002016 0.0022 0.0024015 0.002621 0.0028585 0.0031155 0.003395 0.0036985 0.0040245 0.0043835 0.0047625 0.005141 0.005511 0.005887 0.006297 0.006758 0.00727 0.007839 0.008471 0.009184 0.0099695 0.0108055 0.011686 0.0126375 0.0137105 0.014928 0.016282 0.0177855 0.0194565 0.021371 0.0235095 0.0257935 0.028209 0.030833 0.0338595 0.037323 0.04111 0.0452255 0.049783 0.055009 0.0609785 0.0676135 0.074947 0.083097 0.092204 0.102388 0.113735 0.126297 0.140086 0.155102 0.171327 0.188734 0.20729 0.226949 0.246645 0.266066 0.284872 0.302715 0.319238 0.33667 0.355063 0.374468 0.394942 0.416545 0.43934 0.463392 0.488773 0.515555 0.543816 0.57364 0.605114 0.638328 0.67338 0.710373 0.749416 0.789422 0.828894 0.870338 0.913855)
  
  
    let temp-dist []
    foreach global-life-distribution [
      let month-factor (1 - (1 - ?) ^ 0.125)
      repeat 8 [set temp-dist lput month-factor temp-dist]
      ]
    set global-life-distribution temp-dist
...
to init-age-USA2007
  let census-dist (list 0.069 0.067 0.069 0.071 0.069 0.07 0.065 0.07 0.074 0.076 0.07 0.061 0.047 0.036 0.028 0.025 0.019 0.013)
    ...
    if timescale = "months-erick" [ set age age * 8 + random 8]
    ]
...
to replace  
    ...
    if selective-pressure = "genetic-level" [
      ...
      let ids []
      foreach scores [set ids lput [who] of one-of turtles with [cumulative-score = ?] ids]
      let sum-scores reduce + scores
      set scores map [? / sum-scores] scores
      
      let index random length scores
      while [random-float 1 > item index scores] [
        set index random length scores
      ]
      
      set genetic-strategy [genetic-strategy] of one-of turtles with [who = item index ids]
      ;qq
    ...
    set genetic-strategy replace-item 2 genetic-strategy random-float 1.0
    ;set likelihood-to-rewire random-float 0.01
    set genetic-strategy replace-item 3 genetic-strategy random-float 0.5
    ]
    ...
    if timescale = "months" [set-life-distribution-USA2007-months]
    if timescale = "months-erick" [set-life-distribution-USA2007-months-erick]
end


to set-cumulative-score
  
  ask turtles [
    let x age  
     if x >= length life-distribution [set x length life-distribution - 1]
     
    ifelse cumulative-score = 0 or age = 0 [
      
     
     set cumulative-score  inst-score 
      ]
    [
      
      set cumulative-score (cumulative-score * (age - 1) + inst-score * (1 - item x life-distribution)) / age
...
true
false
"if timescale = \"years\" [set-plot-x-range 0 130]" ""
PENS
